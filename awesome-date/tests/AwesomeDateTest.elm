        ...
        [ fuzz (intRange -400 3000) "determines leap years correctly" <|
            \year ->
                if List.member year validLeapYears then
                    Date.isLeapYear year |> Expect.true "expected leap year"

                else
                    Date.isLeapYear year |> Expect.false "did not expect leap year"
        ]
    ...
    describe "addYears"
        ...
        , fuzz int "changes the year by the amount given" <|
            \years ->
                ...
                in
                (Date.year newDate - Date.year exampleDate) |> Expect.equal years
        ]
